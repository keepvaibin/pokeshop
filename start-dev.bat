...
REM  Expects: manage.py at repo root + frontend/ subfolder
REM ─────────────────────────────────────────────────
...
REM ── Backend (Django) ──────────────────────────────
start "Django Backend [:8000]" cmd /k "cd /d %ROOT% && call %ROOT%.venv\Scripts\activate.bat && pip install -r requirements.txt && python manage.py migrate --noinput && python manage.py runserver 0.0.0.0:8000"

REM Give the backend a moment to start before Next.js dev server
timeout /t 4 /nobreak >nul

REM ── Frontend (Next.js) ────────────────────────────
start "Next.js Frontend [:3000]" cmd /k "cd /d %ROOT%frontend && npm install && npm run dev"
