  ...
  return (
    <div className="space-y-3">
      ...
      {cards.map((card, idx) => {
        ...
        return (
        <div key={idx} className={`border rounded-xl overflow-hidden transition-all ${card.is_wanted_card ? 'border-yellow-300 bg-yellow-50/50' : 'border-gray-200 bg-white'}`}>
          {/* Collapsed header */}
          <div
          ...
          {expandedIdx === idx && (
            <div className="px-4 pb-4 space-y-3 border-t border-gray-100">
              ...
              {hasOracle && conditionAdjusted !== null && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-xs space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-900">${card.base_market_price!.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Condition ({CONDITION_OPTIONS.find(o => o.value === card.condition)?.label}): &times;{condMult}</span>
                    <span className="font-semibold text-gray-900">${conditionAdjusted.toFixed(2)}</span>
                  </div>
              ...
              {hasOracle && (
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Your Expected Value ($) <span className="font-normal text-gray-400">— optional</span></label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
              ...
              <div>
                ...
                {card.photo ? (
                  ...
                  <label className={`flex items-center gap-2 cursor-pointer rounded-lg border border-dashed border-gray-300 bg-gray-50 px-3 py-2 hover:border-blue-400 hover:bg-blue-50 transition-colors w-fit ${compressingIdx === idx ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      {cards.length < maxCards && (
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => addCard()}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-gray-300 rounded-xl text-sm font-medium text-gray-600 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50/50 transition-all"
          >
      ...
      {cards.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-1">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-gray-900">${rawTotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
