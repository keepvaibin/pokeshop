...
export default function AdminWantedPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/inventory/wanted-cards/', formData, {
        headers: {
      ...
      setStatus('success');
      setMessage(`Created wanted card: ${response.data.title}`);
      setTitle('');
      setDescription('');
      setPrice('');
      setImageFiles([]);
    } catch (error) {
      console.error('Wanted card creation failed', error);
      setStatus('error');
      setMessage('Unable to create wanted card. Please check your inputs and try again.');
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
        <div className="max-w-xl w-full bg-white rounded-3xl border border-gray-200 p-10 text-center shadow-lg">
          ...
          <p className="text-gray-600">You must be signed in with a staff or admin account to manage wanted cards.</p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
        <div className="max-w-xl w-full bg-white rounded-3xl border border-gray-200 p-10 text-center shadow-lg">
          ...
          <p className="text-gray-600">Only admin users can use the wanted card management panel.</p>
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-600">Admin Wanted Cards</p>
            <h1 className="mt-3 text-3xl sm:text-4xl font-extrabold text-gray-900">Add a wanted card</h1>
            <p className="mt-2 text-gray-600 max-w-2xl">
              Create a wanted card entry with multiple images for trade offers.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
          <div className="grid gap-6 sm:grid-cols-2">
            <label className="block">
              <span className="text-sm font-semibold text-gray-700">Card Name</span>
              <input
                value={title}
          ...
          <label className="block">
            ...
            <textarea
              ...
              rows={6}
              placeholder="Write a short description for the wanted card."
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              {status === 'saving' ? 'Saving…' : 'Create Wanted Card'}
            </button>
          </div>
        ...
        <div className="mt-8 rounded-3xl border border-dashed border-gray-200 bg-white p-6 text-sm text-gray-600 shadow-sm">
          ...
          <p className="mt-2">Use clear, descriptive card names and upload multiple high-resolution images to attract better trade offers.</p>
        </div>
      </main>
    </div>
  );
