    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <div className="mb-8">
          ...
          <h1 className="mt-3 text-3xl sm:text-4xl font-extrabold text-gray-900 flex items-center gap-2">
        ...
        {/* Create form */}
        <form onSubmit={handleCreate} className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm space-y-5 mb-10">
          ...
          <div className="grid gap-5 sm:grid-cols-3">
            ...
            <div className="block">
              ...
              <label className="mt-2 flex items-center gap-2 cursor-pointer rounded-3xl border border-dashed border-gray-300 bg-gray-50 px-4 py-3 hover:border-blue-400 hover:bg-blue-50 transition-colors">
            ...
            <div className="flex flex-wrap gap-3">
              {imageFiles.map((f, i) => (
                <div key={i} className="relative group">
                  <img src={imageUrls[i]} alt={f.name} className="w-16 h-16 object-cover rounded-xl border border-gray-200" />
            ...
            {cards.map(card => (
              <div key={card.id} className={`bg-white border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow ${!card.is_active ? 'opacity-60' : ''}`}>
                {card.images.length > 0 ? (
                  <FallbackImage src={card.images[0].url} alt={card.name} className="w-full h-40 object-cover" fallbackClassName="w-full h-40 bg-gray-100 flex items-center justify-center" fallbackSize={40} />
                ) : (
                  <div className="w-full h-40 bg-gray-100 flex items-center justify-center"><Star className="w-10 h-10 text-gray-300" /></div>
                )}
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <button
                      ...
                      className={`text-xs px-3 py-1 rounded-full font-semibold transition-colors ${card.is_active ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
